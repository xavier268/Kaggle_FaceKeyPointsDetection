...
Now, we try to allocate a test image to a cluster, and assume the kluster center should match the image.

**Using an euclidian distance, we will score a poor 25,25365 score !**

We changed from basic,"distance" to searching for the best correlation.

This improves slightly the score to **  24.57151 **. Still very poor and inefficient ...

```{r}
kcl.fit <- function(im) {
  im <- c(im)
  d <- apply(kcl.im.mean,1,FUN=function(x){cor(x,im)})
...
```{r computing predictions be patient }
kcl.predict <- matrix(0,nrow(im.test),30)

for(k in 1:nrow(im.test)) {
  kcl.predict[k,] <- kcl$centers[
      kcl.fit(
        im.test[k,]
      )
      ]  
